...
import { createInvite, deleteInvite, logoutAdmin } from "./actions";
import { Copy, Plus, Trash2, CheckCheck, Loader2, LogOut } from "lucide-react";
...
export default function AdminClient({ initialInvites }: { initialInvites: Invite[] }) {
  ...
  const handleCreate = () => {
    ...
    startTransition(async () => {
      const res = await createInvite(label.trim());
      if (res?.token) {
        setInvites((prev) => [
          {
            token: res.token,
